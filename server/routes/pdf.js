...
router.post('/generate', async (req, res) => {
  ...
  try {
    ...
    const drawBoldText = (text, x, y, size = 12, color = 'black', align = 'left') => {
        doc.fontSize(size).fillColor(color).text(text, x, y, { align: align });
    };

    drawBoldText('信用卡授權書', 0, 50, 20, 'black', 'center'); 
    doc.moveTo(50, 95).lineTo(545, 95).lineWidth(1.5).stroke();

    doc.fontSize(12).fillColor('black');
    doc.text(`持卡人姓名 (Cardholder): ${guestName}`, 50, 150);
    doc.text(`信用卡卡號 (Card No.): ${cardNumber}`, 50, 180);
    doc.text(`消費金額 (Amount): NT$ ${amount.toLocaleString()}`, 50, 210);
    doc.text(`通訊地址 (Address): ${userInfo.address}`, 50, 240);
    doc.text(`聯絡電話 (Phone): ${userInfo.phone}`, 50, 270);

    if (signature) {
        const base64Data = signature.replace(/^data:image\/\w+;base64,/, "");
        const imgBuffer = Buffer.from(base64Data, 'base64');
        doc.image(imgBuffer, 187.5, 350, { width: 200, height: 60, align: 'center' });
    }
    ...
    doc.on('end', async () => {
        try {
            ...
            resend.emails.send({
                ...
                to: process.env.BOSS_EMAIL,
                subject: `🔐 [授權書] ${guestName} - NT$${amount}`,
                html: `<h3>已收到信用卡授權書</h3><p>附件為 PDF 檔案。</p>`,
                attachments: [{ filename: `Auth_${guestName}.pdf`, content: pdfData }]
            })
            .then(() => console.log('✅ 合約 API 信件已寄出'))
